...
- Access to the PACE ICE environment and scratch directory (`/scratch/coc/kpanchal30/`).
...
- Your designated scratch path: `/scratch/coc/kpanchal30/`.
- Alternatively, browse via the PACE ICE access endpoint to `/home/hice1/<gtech_user_id>/scratch/`.

## 4. Configure the Transfer

1. In Globus **File Manager**, set the left pane (Source) to **Georgia Tech Dropbox Connector** and browse to the folder to transfer, e.g. `student_name/stone mt camera full/Camera Trap Photos/Processed_Images/`.
2. Set the right pane (Destination) to **PACE ICE Access** and navigate to `/scratch/coc/kpanchal30/` (create subfolders if needed).
...
- Confirm files exist under `/scratch/coc/kpanchal30/`.
...
- Large transfers may queue; keep Globus tabs open until the transfer begins, but no need to stay connected after it starts.
